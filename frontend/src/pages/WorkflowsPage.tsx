  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : t('workflows.errorLoad'));
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
    ...
    <div className="p-6 space-y-6">
      ...
      <section className="grid grid-cols-2 gap-4">
        <div className="p-4 space-y-2">
          <div className="text-xs uppercase tracking-[0.16em] text-muted">{t('workflows.metricSessions')}</div>
          <div className="text-3xl font-bold font-serif leading-none">{workflows.length}</div>
          <div className="text-xs text-muted">{t('workflows.metricSessionsHint')}</div>
        </div>
        <div className="p-4 space-y-2">
          <div className="text-xs uppercase tracking-[0.16em] text-muted">{t('workflows.metricAverageSuccess')}</div>
          <div className="text-3xl font-bold font-serif leading-none">{averageSuccess}%</div>
          <div className="text-xs text-muted">{t('workflows.metricAverageSuccessHint')}</div>
        </div>
      </section>

      {loading ? <div className="text-sm text-muted">{t('workflows.loading')}</div> : null}
      {error ? <div className="text-sm text-danger">{error}</div> : null}

      {!loading && !error && filtered.length === 0 ? (
        <EmptyState title={t('workflows.noSessionsTitle')} description={t('workflows.noSessionsDescription')} />
      ) : null}
            ...
            <Link key={workflow.id} to={`/workflows/${encodeURIComponent(workflow.id)}`} className="record-card bg-surface block p-4 space-y-3 hover:border-primary transition-colors">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-xs text-muted">{t('workflows.success')}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 text-xs text-muted">
                <div>{t('workflows.steps', { count: workflow.total_steps })}</div>
                <div>{t('workflows.agentActions', { count: workflow.agent_action_count })}</div>
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  ...
                  {workflow.selected_skills.length > 3 ? (
                    <span className="tag px-2 py-1 text-muted">
                      {t('common.more', { count: workflow.selected_skills.length - 3 })}
                    </span>
                  ) : null}
                </div>
              ) : null}
