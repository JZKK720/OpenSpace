  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : t('skills.errorLoad'));
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="flex gap-3 items-center">
          ...
          />
          <select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)} className="px-3 py-2" title={t('skills.sortMenu')}>
            <option value="score">{t('skills.sortScore')}</option>
            <option value="updated">{t('skills.sortUpdated')}</option>
      ...
      {stats ? (
        <section className="metrics-row">
          <MetricCard label={t('skills.metricClasses')} value={skillClasses.length} hint={t('skills.metricClassesHint', { count: stats.total_skills_all })} />
          <MetricCard label={t('skills.metricActiveVersions')} value={totalActiveVersions} hint={t('skills.metricActiveVersionsHint', { count: stats.skills_with_activity })} />
          <MetricCard label={t('skills.metricAverageBestScore')} value={averageBestScore.toFixed(1)} hint={t('skills.metricAverageBestScoreHint')} />
          <MetricCard label={t('skills.metricSelections')} value={stats.total_selections} hint={t('skills.metricSelectionsHint', { count: stats.total_completions })} />
        </section>
      ) : null}

      {loading ? <div className="text-sm text-muted">{t('skills.loading')}</div> : null}
      {error ? <div className="text-sm text-danger">{error}</div> : null}

      {!loading && !error && filteredClasses.length === 0 ? (
        <EmptyState title={t('skills.noMatchTitle')} description={t('skills.noMatchDescription')} />
      ) : null}
          ...
          {filteredClasses.map((skillClass) => (
            ...
            >
              ...
              <div className="grid grid-cols-4 gap-3 text-xs text-muted">
                ...
                <div>{t('skills.activeVersions', { count: skillClass.active_count })}</div>
                <div>{t('skills.selections', { count: skillClass.total_selections })}</div>
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                {skillClass.tags.length > 5 ? (
                  <span className="tag px-2 py-1">{t('common.tagsMore', { count: skillClass.tags.length - 5 })}</span>
                ) : null}
              </div>
            </Link>
          ))}
