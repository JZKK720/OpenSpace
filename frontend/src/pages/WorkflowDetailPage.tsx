...
import { getDocumentLocale, useI18n } from '../i18n';
...
export default function WorkflowDetailPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : t('workflowDetail.errorLoad'));
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
    ...
    return (
      <div className="workflow-detail-page p-6">
        <div className="mx-auto max-w-[1480px]">
          <div className="workflow-panel p-6 text-sm text-muted">{t('workflowDetail.loading')}</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !workflow) {
    return (
      <div className="workflow-detail-page p-6">
        <div className="mx-auto max-w-[1480px]">
          <div className="workflow-panel p-6 text-sm text-danger">{error ?? t('workflowDetail.notFound')}</div>
        </div>
      </div>
    );
  }
  ...
  const selectedSkillLabel = t('workflowDetail.selectedSkillsLabel', { count: workflow.selected_skills.length });
  const iterationsLabel = t('workflowDetail.iterationsLabel', { count: workflow.iterations });
  const totalStepLabel = t('workflowDetail.stepsLabel', { count: workflow.total_steps });
  const actionCountLabel = t('workflowDetail.agentActionsLabel', { count: workflow.agent_action_count });
  ...
  const latestEventLabel = timelineSummary.lastTimestamp ? formatTimeLabel(timelineSummary.lastTimestamp) : t('workflowDetail.latestEventFallback');
  const timelineEventLabel = t('workflowDetail.mergedEventsLabel', { count: timelineSummary.total });
      ...
      <div className="mx-auto max-w-[1480px] space-y-6">
        <section className="workflow-hero p-6 lg:p-8 space-y-8">
          <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
            <div className="min-w-0 flex-1 space-y-5">
              ...
              <div className="space-y-3">
                <div className="workflow-kicker">{t('workflowDetail.kicker')}</div>
            ...
            <div className="workflow-soft-card w-full max-w-sm shrink-0 p-5 space-y-5">
              <p className="workflow-copy text-base leading-7 text-muted">
                {t('workflowDetail.summary', { status: statusLabel, iterations: iterationsLabel, actions: actionCountLabel })}
              </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1">
                  <div className="workflow-kicker">{t('workflowDetail.steps')}</div>
          ...
          <section className="workflow-metrics-row">
            ...
            <SummaryMetric
              ...
              hint={t('workflowDetail.iterationsHint', { duration: executionDurationLabel })}
            />
            <SummaryMetric
              label={t('workflowDetail.activeBackends')}
              value={activityEntries.length}
              hint={topBackendEntry ? t('workflowDetail.activeBackendsHint', { backend: humanizeToken(topBackendEntry[0]), count: topBackendEntry[1] }) : t('workflowDetail.noToolActivity')}
            />
            <SummaryMetric
              label={t('workflowDetail.timelineEvents')}
              value={timelineSummary.total}
              hint={t('workflowDetail.timelineEventsHint', { agentActions: agentActionCount, toolEvents: toolExecutionCount })}
            />
          </section>
        </section>
        ...
        <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.72fr)]">
          <div className="workflow-panel p-5 space-y-4">
            {timeline.length === 0 ? (
              <QuietEmptyState
                title={t('workflowDetail.noTimelineTitle')}
                description={t('workflowDetail.noTimelineDescription')}
              />
            ) : (
            ...
            <section className="workflow-panel p-5 space-y-4">
              ...
              {workflow.selected_skills.length > 0 ? (
                ...
                <div className="space-y-2">
                  <div className="text-lg font-semibold tracking-[-0.02em] text-ink">{t('workflowDetail.noSelectedSkillsTitle')}</div>
                  <p className="workflow-copy text-sm leading-6 text-muted">{t('workflowDetail.noSelectedSkillsDescription')}</p>
                </div>
              )}
            </section>
            ...
            <section className="workflow-panel p-5 space-y-5">
              ...
              <div className="space-y-5">
                ...
                <SidebarRow label={t('workflowDetail.window')}>
                  ...
                  <div className="text-xs leading-6 text-muted">{t('workflowDetail.ended', { time: formatDate(workflow.end_time) })}</div>
                </SidebarRow>
