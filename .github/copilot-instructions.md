...
- When the user is brainstorming how an external harness or operator system such as ECC could plug into OpenSpace, start with the [ECC Integration agent](./agents/ecc-integration.agent.md) or [Plan ECC Integration prompt](./prompts/ecc-integration.prompt.md). Classify the idea as a delegated runtime, shared MCP surface, standalone utility app, curated workflow import, or thin sidecar adapter before suggesting any code import, and prefer OpenSpace as the control plane over a raw cross-repo merge.
- When the ECC idea becomes a concrete endpoint or registry design question, use the [Plan ECC Adapter Contract prompt](./prompts/ecc-adapter-contract.prompt.md) before editing bridge files. When the goal is to reuse ECC workflow ideas without importing its harness, use the [ecc-workflow-curation skill](./skills/ecc-workflow-curation/SKILL.md) and translate only the narrow user-facing workflow into an OpenSpace-native surface.
