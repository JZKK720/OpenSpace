...
- External runtime swaps such as Nanobot to OpenClaw are coordinated migrations across `openspace/config/external_agents.json`, `.env*`, Compose, gateway modules, docs, and `smoke_test_mcp.py`. Start with the [External Agent Migration agent](./agents/external-agent-migration.agent.md) or [Plan External Agent Migration prompt](./prompts/external-agent-migration.prompt.md) before editing those surfaces, and check port changes against `7788`, `8788`, and `5173`.
