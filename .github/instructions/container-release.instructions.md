...
- **GHCR auto-trigger**: `ghcr-release.yml` fires automatically on push to `main` (publishes `main`-channel + `sha-<shortsha>` immutable tags) and on push of a `v*` tag (also publishes a versioned release). No manual `workflow_dispatch` is needed after a routine sync-and-push. When both an upstream sync and a GHCR release are in scope for the same session, use the [Sync Upstream and Release prompt](../prompts/sync-and-release.prompt.md) to chain sync → validate → push → optional version tag in one workflow.
- **Versioned releases**: to publish a `vX.Y.Z` tagged image set and a GitHub Release, push the tag after validating on `origin/main`: `git tag vX.Y.Z && git push origin vX.Y.Z`. Confirm the tag name with the user before pushing; never push a version tag that collides with an existing published release without a warning.